...
class MotionProfileExample
{
   public:
      ...
      MotionProfileExample(WPI_TalonSRX & talon) :
      ...
      void control()
      {
         ...
         if (_loopTimeout < 0) {
            ...
            if (_loopTimeout == 0) {
                ...
                * tripped
                */
               DriverStation::ReportError ("No progress for ID" + _talon.GetDeviceID ());
            }
            else {
               --_loopTimeout;
            }
      ...
      TrajectoryDuration GetTrajectoryDuration(int durationMs)
      {
         ...
         DriverStation::ReportError ("Trajectory Duration not supported - use configMotionProfileTrajectoryPeriod instead\n");
         return TrajectoryDuration_100ms;
      }
      ...
      void startFilling(const double profile[][3], int totalCnt)
      {
         ...
         if (_status.hasUnderrun) {
            ...
            DriverStation::ReportError (_talon.GetDeviceID () + " has Underrun!");
            /*
         ...
         for (int i = 0; i < totalCnt; ++i) {
            ...
            point.position = (phase ? -1 : 1) * positionRot * kTicksPerFoot; //Convert Revolutions to Units
            point.velocity = (phase ? -1 : 1) * velocityRPM * kTicksPerFoot / 600.0; //Convert RPM to Units/100ms
